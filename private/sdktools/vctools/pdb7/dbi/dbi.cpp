    ...
    *pimod = -1;
    ...
    if (pbscLo != NULL) {
        if (pbscEnd - pbscLo > 0) {
            ...
            key.isect = isect;
            key.off   = off;
            psc = reinterpret_cast<T *>(
                            bsearch(&key, 
                                pbscLo, 
                                (pbscEnd - pbscLo) / sizeof(T), sizeof(T),
...
BOOL DBI1::fUpdateTmpct(UTFSZ_CONST utfszModule, UTFSZ_CONST utfszInternalName, TM* ptm)
{
    ...
    for (; potm; potm = potm->pNext) {
        if (potm->tiPreComp == tiPreComp &&
            IsEqualSig70(potm->sig70, sig70Match) &&
...
bool DBI1::SaveFrameData()
{
    ...
    if (m_rgFrameData.size() > 0) {
        ...
        if (m_rgrvaRemovals.size() > 0) {
            qsort(m_rgrvaRemovals.pBase(), 
                m_rgrvaRemovals.size(), 
                sizeof MemBlock, 
                (int (__cdecl*)(const void*, const void*)) blkCompare);
            ...
            qsort(m_rgFrameData.pBase(), 
                m_rgFrameData.size(), 
                sizeof FRAMEDATA, 
                (int (__cdecl*)(const void*, const void*)) frameCompare);
